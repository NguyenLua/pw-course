...
import { CategoriesPage } from './page/category-page-pw';
import { HomePage } from './page/home-page-pw';
import { POMManager } from './page/pom-manager-pw';
import { TagsPage } from './page/tags-page-pw';
...
test.describe('POST - Post/Tags', async () => {
    ...
    test('@POST_TAG_001 - Tag - add tag failed', async ({ page }) => {
        await test.step('Click button "Add New Tag"', async () => {
            ...
            //result
            await expect(tagsPage.page.locator(tagsPage.xpathNoticeError)).toContainText(errorMessageRequiredField);
        });
        ...
        await test.step('Điền thông tin tag: name = "lesson tag", click button "Add New Tag', async () => {
            ...
            //result
            await expect(tagsPage.page.locator(tagsPage.xpathNoticeError)).toContainText(errorMessageDuplicate);
        });
    });
...
test.describe('POST - Post/Category', async () => {
    ...
    test.beforeEach(async ({ page }) => {
        pomManager = new POMManager(page);
        homePage = pomManager.getHomePage();
        categoriesPage = pomManager.getCategoryPage();
    ...
    test('@POST_CATEGORY_001 - Category - create category success', async ({ page }) => {
        await test.step('Điền thông tin category: name = "category {name} 03", slug = "Đây là category đặc biệt @100 $200"', async () => {
            ...
            //Result
            await expect(categoriesPage.page.locator(categoriesPage.xpathNoticeSuccess)).toContainText(noticeSuccess);
        });
